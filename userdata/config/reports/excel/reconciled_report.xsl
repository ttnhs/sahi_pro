		...
		<script type="text/javascript">
				...
				function getHeader(){
					return "<graph caption=\'Load test summary\'  xAxisName=\'Loads\' yAxisName=\'Time (ms)\' animation='0' showValues=\'0\'" + 
						   ...
						   " bgColor=\'E9E9E9\' canvasBorderThickness=\'0\' decimalPrecision=\'0\' showLegend='1'>";
				}
				...
				function getDataset(runs, steps){
					...
					for(var i=0; i<steps; i++){
						dataset[i] += "<dataset seriesName=\'" + headerTable.rows[i].cells[0].textContent
							.replace(/&/g, "&amp;").replace(/'/g, '&apos;').replace(/"/g, '&quot;')
							.replace(/</g, "&lt;").replace(/>/, "&gt;")
							+ "\' color=\'"+ arr_FCColors[++FC_ColorCounter % arr_FCColors.length] + "\'>";
