...
java -cp $SAHI_HOME/lib/ant-sahi.jar in.co.sahi.distributed.DLoadRunner -origScriptsPath $ORIG_SCRIPTS_UDPATH -copiedScriptsPath $COPIED_SCRIPTS_PATH -noise $COPIED_SCRIPTS_PATH/$1 -noiseBrowserType phantomjs -min 1 -max 9 -incrementBy 2 -interval 5 -subject $COPIED_SCRIPTS_PATH/$2 -subjectRepeatCount 3 -browserType $4 -baseURL "$START_URL" -host localhost -port 9999 -nodes "$NODES" -ignorePattern ".*(svn|copied).*"
fi
