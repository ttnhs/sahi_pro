...
function changeControllerMode(){
	...
	alert("Your Controller mode has been set to " + mode + ". Please restart your controller after refreshing your browser.");
}
Controller = function(){
		...
		try{ 
		 	if (msg.mode == "PLAYBACK_LOG_REFRESH"){
				...
				if (msg.accessor && msg.mode != "RE_ID") {
		 		...
		 		//this.populateOptions($("alternative"), this.language.translateArrayFromSahi(msg.alternative));
		 		this.populateOptions($("alternative"), msg.alternative);
				$("elValue").value = (msg.value) ? msg.value : ""; 
				$("prefix").value = (msg.winName) ? msg.winName : ""; 
...
Controller.prototype.makeJavaAccessor = function(accessor, winName){
	...
	accessor = (winName == "" ? "" : winName) + accessor; 
	return accessor;
};
...
Controller.prototype.getAccessor = function(){
	var accessor = $("accessor").value;
	if(getControllerMode() == "ruby")
		accessor = accessor.replace(/\@b\./g,'b.');
	var v = this.sendToServer("/_s_/dyn/ControllerUI_translateJavaToSahi?java=" + encodeURIComponent(accessor));
	return v;
}
...
Controller.prototype.setValue = function(){
	...
	var s = "_sahi._setValue(" + el + ", " + this.quote($("elValue").value) + ")";
	this.sendMessageToBrowser(s);	
};
...
function resizeTAs() {
	var t = findPos(getScroller('result'))[1];
	var r = findPos(getScroller('lastSteps'))[1];
	var s = findPos(getScroller('allsteps'))[1];
	if (t > 10) {
		resizeTA2(getScroller('result'), 22, t, 60);
	}
	if (r > 10) {
		resizeTA2(getScroller('lastSteps'), 22, r + 75, 50);
	}
	if (s > 10) {
		resizeTA2(getScroller('allsteps'), 25, s, 85);
	}
}
...
Controller.prototype.sendMessageToBrowser = function (s){
	//alert(s);
	sendMessage(s, "result", true);
	//return sahi()._eval(sahi().addSahi(s));
};
function sahi(){
...
Controller.prototype.saveState = function (){
    var ids = ["accessor", "elValue", "result", "pbScript", "pbStartURL", "current_script", "currentLogFileName", "record_script"];
    var s = [];
...
Controller.prototype.appendToScript= function(){
	var s = this.getSelectedText($("lastSteps"));
	this.sendToServer("/_s_/dyn/StepWiseRecorder_record?step="+ encodeURIComponent(s));
};
Controller.prototype.evaluateExpr = function(s){
	try{
		if (!s) s = this.getSelectedText($("lastSteps"));
	    var v = this.sendToServer("/_s_/dyn/ControllerUI_translateJavaToSahi?java=" + encodeURIComponent(s));
//	    alert(v);
	    $('result').value = this.sendMessageToBrowser(v);
	}catch(e){
//		alert(e.message);
	}
}
Controller.prototype.demoClick = function(){
	this.evaluateExpr(this.getSelectedText($("lastSteps")) + ".click()");
};
Controller.prototype.demoHighlight = function(){
	this.evaluateExpr(this.getSelectedText($("lastSteps")) + ".highlight()");
};
...
java.translateToSahi = function(s){
	if(getControllerMode() == "ruby")
		s = s.replace(/\@b\./g,'b.');
	return s.replace(/^b[.]/, "_");
};
_c.language = java;
Controller.prototype.displayInfo = function(alternative, accessor, value, winName, assertions){
	$("accessor").value = alternative[0];
	this.populateOptions($("alternative"), alternative);
	$("elValue").value = (value) ? value : ""; 
	$("prefix").value = (winName) ? winName : ""; 
...
function getControllerMode() {
	return sahiSendToServer("/_s_/dyn/Configuration_getControllerMode");
}
